  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You provide a very simple, one-sentence summary of the user's text."
        },
        {
          role: "user",
          content: text
          // just the raw text, no extra instruction needed here
        }
      ]
    });
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, ([tab]) => {
      ...
      chrome.scripting.executeScript(
        ...
        async (results) => {
          ...
          } else if (selectedText.length > 1e3) {
